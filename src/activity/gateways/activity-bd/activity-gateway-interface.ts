...
export interface ActivityGatewayInterface{
    ...
    count(disciplineId: number, partialName: string, typeAc: string): Promise<number>;
    findAll(disciplineId: number, partialName: string, typeAc: string): Promise<Activity[]>;
    findAllWithPagination(disciplineId: number, page: number, limit: number, partialName: string, typeAc: string): Promise<Activity[]>;
